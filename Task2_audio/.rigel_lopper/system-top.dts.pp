# 0 "C:\\Users\\kurti\\Downloads\\Soc_FinalProject\\Task2_audio\\platform\\hw\\sdt\\system-top.dts"
# 0 "<built-in>"
# 0 "<command-line>"
# 1 "C:\\Users\\kurti\\Downloads\\Soc_FinalProject\\Task2_audio\\platform\\hw\\sdt\\system-top.dts"
/dts-v1/;
# 1 "C:\\Users\\kurti\\Downloads\\Soc_FinalProject\\Task2_audio\\platform\\hw\\sdt\\pl.dtsi" 1
/ {
 cpus_microblaze_riscv_0: cpus_microblaze_riscv@0 {
  #cpu-mask-cells = <1>;
  compatible = "cpus,cluster";
  #address-cells = <1>;
  #size-cells = <0>;
  microblaze_riscv_0: cpu@0 {
   model = "microblaze_riscv,1.0";
   xlnx,d-axi = <1>;
   xlnx,pmp-entries = <0>;
   xlnx,interrupt-mon = <0>;
   xlnx,iaddr-size = <32>;
   xlnx,number-of-wr-addr-brk = <0>;
   xlnx,rable = <0>;
   xlnx,temporal-depth = <0>;
   xlnx,use-interrupt = <0>;
   xlnx,optimization = <0>;
   xlnx,ip-axi-mon = <0>;
   d-cache-highaddr = <0x3fffffff>;
   xlnx,ip-name = "microblaze_riscv";
   xlnx,dcache-force-tag-lutram = <0>;
   xlnx,pc-width = <17>;
   xlnx,interrupt-is-edge = <0>;
   reg = <0x0>;
   xlnx,async-interrupt = <1>;
   xlnx,use-mmu = <0>;
   xlnx,icache-victims = <0>;
   xlnx,use-bitman-a = <0>;
   xlnx,d-lmb-mon = <0>;
   xlnx,d-lmb-protocol = <0>;
   xlnx,use-bitman-b = <0>;
   xlnx,use-bitman-c = <0>;
   xlnx,dc-axi-mon = <0>;
   xlnx,debug-trace-async-reset = <0>;
   xlnx,trace = <0>;
   xlnx,part = "xc7s50csga324-1";
   xlnx,use-config-reset = <0>;
   xlnx,i-lmb-mon = <0>;
   xlnx,dcache-byte-size = <8192>;
   xlnx,ill-instr-exception = <2>;
   compatible = "xlnx,microblaze-riscv-1.0" , "xlnx,microblaze_riscv";
   xlnx,data-size = <32>;
   xlnx,fault-tolerant = <0>;
   xlnx,use-sleep = <0>;
   xlnx,debug-trace-size = <0>;
   xlnx,pmp-granularity = <2>;
   xlnx,mmu-privileged-instr = <0>;
   d-cache-line-size = <16>;
   xlnx,d-lmb = <1>;
   xlnx,enable-discrete-ports = <0>;
   clock-frequency = <100000000>;
   xlnx,debug-interface = <0>;
   xlnx,use-ext-brk = <0>;
   xlnx,daddr-size = <32>;
   xlnx,debug-enabled = <1>;
   xlnx,num-sync-ff-dbg-trace-clk = <2>;
   xlnx,i-lmb-protocol = <0>;
   xlnx,fsl-exception = <0>;
   xlnx,use-extended-fsl-instr = <0>;
   xlnx,dp-axi-mon = <0>;
   xlnx,impid = <0x1>;
   device_type = "cpu";
   xlnx,branch-target-cache-size = <0>;
   xlnx,dcache-use-writeback = <0>;
   xlnx,use-bitman-s = <0>;
   bus-handle = <&amba_pl>;
   xlnx,icache-line-len = <4>;
   xlnx,base-vectors = <0x0>;
   xlnx,ecc-use-ce-exception = <0>;
   xlnx,edk-special = "microblaze_riscv";
   xlnx,use-dcache = <0>;
   xlnx,use-barrel = <1>;
   xlnx,use-muldiv = <0>;
   xlnx,addr-size = <32>;
   xlnx,debug-external-trace = <0>;
   xlnx,piaddr-size = <32>;
   xlnx,misaligned-exceptions = <1>;
   xlnx,use-bitman = <0>;
   xlnx,num-sync-ff-clk-debug = <2>;
   xlnx,debug-event-counters = <0>;
   xlnx,use-atomic = <0>;
   xlnx,i-axi = <0>;
   xlnx,g-template-list = <0>;
   xlnx,icache-streams = <0>;
   xlnx,dcache-line-len = <4>;
   xlnx,num-sync-ff-clk = <2>;
   xlnx,edk-iptype = "PROCESSOR";
   xlnx,num-sync-ff-dbg-clk = <1>;
   xlnx,interconnect = <2>;
   xlnx,hartid = <0x0>;
   xlnx,debug-latency-counters = <0>;
   xlnx,use-fpu = <0>;
   xlnx,icache-byte-size = <8192>;
   xlnx,edge-is-positive = <1>;
   xlnx,use-icache = <0>;
   d-cache-size = <8192>;
   xlnx,memory-ip-list = "microblaze_riscv_0_local_memory_dlmb_bram_if_cntlr_memory" , "microblaze_riscv_0_local_memory_ilmb_bram_if_cntlr_memory";
   xlnx,async-wakeup = <3>;
   xlnx,use-non-secure = <0>;
   xlnx,s-axi = <0>;
   xlnx,number-of-rd-addr-brk = <0>;
   d-cache-baseaddr = <0x0>;
   xlnx,use-compression = <0>;
   xlnx,avoid-primitives = <0>;
   xlnx,use-counters = <1>;
   xlnx,lockstep-slave = <0>;
   xlnx,use-ext-nm-brk = <0>;
   xlnx,pdaddr-size = <32>;
   xlnx,instr-size = <32>;
   xlnx,i-lmb = <1>;
   xlnx,lockstep-select = <0>;
   xlnx,lmb-data-size = <32>;
   xlnx,ic-axi-mon = <0>;
   xlnx,num-sync-ff-clk-irq = <1>;
   xlnx,fsl-links = <0>;
   timebase-frequency = <100000000>;
   xlnx,archid = <0x1>;
   xlnx,icache-force-tag-lutram = <0>;
   xlnx,dcache-data-width = <0>;
   xlnx,dcache-victims = <0>;
   xlnx,use-branch-target-cache = <0>;
   xlnx,debug-profile-size = <0>;
   xlnx,lockstep-master = <0>;
   xlnx,freq = <100000000>;
   xlnx,number-of-pc-brk = <1>;
   xlnx,imprecise-exceptions = <0>;
   xlnx,icache-data-width = <0>;
   xlnx,debug-counter-width = <64>;
  };
 };
 clock: clocks {
  #address-cells = <1>;
  #size-cells = <0>;
  clk_cpu_0: clk_cpu@0 {
   compatible = "fixed-clock";
   reg = <0>;
   clock-frequency = <100000000>;
   clock-output-names = "clk_cpu";
   #clock-cells = <0>;
  };
  clk_bus_0: clk_bus_0@1 {
   compatible = "fixed-clock";
   reg = <1>;
   clock-frequency = <100000000>;
   clock-output-names = "clk_bus_0";
   #clock-cells = <0>;
  };
 };
 amba_pl: amba_pl {
  ranges;
  compatible = "simple-bus";
  #address-cells = <1>;
  #size-cells = <1>;
  apb_led_ctl_0: apb_led_ctl@44a00000 {
   xlnx,rable = <0>;
   compatible = "xlnx,apb-led-ctl-1.0";
   status = "okay";
   xlnx,ip-name = "apb_led_ctl";
   xlnx,edk-iptype = "PERIPHERAL";
   reg = <0x44a00000 0x10000>;
   xlnx,name = "apb_led_ctl_0";
  };
  axi_gpio_0: gpio@40000000 {
   xlnx,gpio-board-interface = "Custom";
   compatible = "xlnx,axi-gpio-2.0" , "xlnx,xps-gpio-1.00.a";
   xlnx,all-outputs = <0>;
   #gpio-cells = <2>;
   xlnx,gpio-width = <2>;
   clock-frequency = <100000000>;
   xlnx,rable = <0>;
   xlnx,dout-default = <0x0>;
   xlnx,is-dual = <0>;
   xlnx,ip-name = "axi_gpio";
   xlnx,tri-default-2 = <0xffffffff>;
   reg = <0x40000000 0x10000>;
   xlnx,all-inputs-2 = <0>;
   clocks = <&clk_bus_0>;
   xlnx,all-outputs-2 = <0>;
   gpio-controller;
   xlnx,interrupt-present = <0>;
   xlnx,gpio2-board-interface = "Custom";
   xlnx,edk-iptype = "PERIPHERAL";
   xlnx,dout-default-2 = <0x0>;
   status = "okay";
   xlnx,gpio2-width = <32>;
   clock-names = "s_axi_aclk";
   xlnx,tri-default = <0xffffffff>;
   xlnx,name = "axi_gpio_0";
   xlnx,all-inputs = <0>;
  };
  axi_timer_0: timer@41c00000 {
   compatible = "xlnx,axi-timer-2.0" , "xlnx,xps-timer-1.00.a";
   xlnx,gen1-assert = <1>;
   clock-frequency = <100000000>;
   xlnx,trig0-assert = <1>;
   xlnx,rable = <0>;
   xlnx,count-width = <32>;
   xlnx,ip-name = "axi_timer";
   xlnx,one-timer-only = <0>;
   reg = <0x41c00000 0x10000>;
   clocks = <&clk_bus_0>;
   xlnx,gen0-assert = <1>;
   xlnx,mode-64bit = <0>;
   xlnx,edk-iptype = "PERIPHERAL";
   status = "okay";
   xlnx,trig1-assert = <1>;
   xlnx,enable-timer2 = <1>;
   xlnx,name = "axi_timer_0";
  };
  axi_uartlite_0: serial@40600000 {
   compatible = "xlnx,axi-uartlite-2.0" , "xlnx,xps-uartlite-1.00.a";
   clock-frequency = <100000000>;
   xlnx,uartlite-board-interface = "Custom";
   xlnx,s-axi-aclk-freq-hz-d = <100>;
   xlnx,rable = <0>;
   xlnx,ip-name = "axi_uartlite";
   reg = <0x40600000 0x10000>;
   xlnx,baudrate = <9600>;
   clocks = <&clk_bus_0>;
   current-speed = <9600>;
   xlnx,use-parity = <0>;
   xlnx,edk-iptype = "PERIPHERAL";
   xlnx,odd-parity = <0>;
   status = "okay";
   xlnx,name = "axi_uartlite_0";
   xlnx,data-bits = <8>;
   xlnx,parity = "No_Parity";
  };
  microblaze_riscv_0_local_memory_dlmb_bram_if_cntlr: lmb_bram_if_cntlr@0 {
   xlnx,edk-special = "BRAM_CTRL";
   xlnx,write-access = <2>;
   compatible = "xlnx,lmb-bram-if-cntlr-4.0";
   xlnx,ecc-onoff-register = <0>;
   xlnx,ecc-onoff-reset-value = <1>;
   xlnx,s-axi-ctrl-protocol = "AXI4LITE";
   xlnx,mask = <0x40020000>;
   xlnx,mask1 = <0x800000>;
   xlnx,rable = <0>;
   xlnx,mask2 = <0x800000>;
   xlnx,fault-inject = <0>;
   xlnx,mask3 = <0x800000>;
   xlnx,arbitration = <0>;
   xlnx,num-lmb = <1>;
   xlnx,mask4 = <0x800000>;
   reg = <0x00000000 0x20000>;
   xlnx,s-axi-ctrl-addr-width = <32>;
   xlnx,mask5 = <0x800000>;
   xlnx,ecc-status-registers = <0>;
   xlnx,mask6 = <0x800000>;
   xlnx,lmb-protocol = <0>;
   xlnx,ce-counter-width = <0>;
   xlnx,mask7 = <0x800000>;
   xlnx,ecc = <0>;
   xlnx,edk-iptype = "PERIPHERAL";
   xlnx,lmb-dwidth = <32>;
   xlnx,ce-failing-registers = <0>;
   xlnx,interconnect = <0>;
   xlnx,ue-failing-registers = <0>;
   xlnx,data-width = <32>;
   xlnx,s-axi-ctrl-data-width = <32>;
   xlnx,bram-awidth = <32>;
   xlnx,lmb-awidth = <32>;
  };
  microblaze_riscv_0_local_memory_ilmb_bram_if_cntlr: lmb_bram_if_cntlr@1 {
   xlnx,edk-special = "BRAM_CTRL";
   xlnx,write-access = <2>;
   compatible = "xlnx,lmb-bram-if-cntlr-4.0";
   xlnx,ecc-onoff-register = <0>;
   xlnx,ecc-onoff-reset-value = <1>;
   xlnx,s-axi-ctrl-protocol = "AXI4LITE";
   xlnx,mask = <0x0>;
   xlnx,mask1 = <0x800000>;
   xlnx,rable = <0>;
   xlnx,mask2 = <0x800000>;
   xlnx,fault-inject = <0>;
   xlnx,mask3 = <0x800000>;
   xlnx,arbitration = <0>;
   xlnx,num-lmb = <1>;
   xlnx,mask4 = <0x800000>;
   reg = <0x00000000 0x20000>;
   xlnx,s-axi-ctrl-addr-width = <32>;
   xlnx,mask5 = <0x800000>;
   xlnx,ecc-status-registers = <0>;
   xlnx,mask6 = <0x800000>;
   xlnx,lmb-protocol = <0>;
   xlnx,ce-counter-width = <0>;
   xlnx,mask7 = <0x800000>;
   xlnx,ecc = <0>;
   xlnx,edk-iptype = "PERIPHERAL";
   xlnx,lmb-dwidth = <32>;
   xlnx,ce-failing-registers = <0>;
   xlnx,interconnect = <0>;
   xlnx,ue-failing-registers = <0>;
   xlnx,s-axi-ctrl-data-width = <32>;
   xlnx,bram-awidth = <32>;
   xlnx,lmb-awidth = <32>;
  };
  myip_pong2_0: myip_pong2@20000 {
   xlnx,rable = <0>;
   xlnx,s00-axi-data-width = <32>;
   compatible = "xlnx,myip-pong2-1.0";
   status = "okay";
   xlnx,s00-axi-addr-width = <4>;
   xlnx,ip-name = "myip_pong2";
   xlnx,edk-iptype = "PERIPHERAL";
   reg = <0x00020000 0x1000>;
   xlnx,name = "myip_pong2_0";
  };
 };
};
# 3 "C:\\Users\\kurti\\Downloads\\Soc_FinalProject\\Task2_audio\\platform\\hw\\sdt\\system-top.dts" 2
/ {
 device_id = "7s50";
 #address-cells = <1>;
 #size-cells = <1>;
 slrcount = <1>;
 family = "microblaze_riscv";
 microblaze_riscv_0_local_memory_dlmb_bram_if_cntlr_memory: memory@0 {
  compatible = "xlnx,lmb-bram-if-cntlr-4.0";
  xlnx,ip-name = "lmb_bram_if_cntlr";
  device_type = "memory";
  memory_type = "memory";
  reg = <0x00000000 0x20000>;
 };
 chosen {
  stdout-path = "serial0:9600n8";
 };
 aliases {
  serial0 = &axi_uartlite_0;
 };
 cpus_microblaze_riscv_0: cpus_microblaze_riscv@0 {
  address-map = <0x00000000 &microblaze_riscv_0_local_memory_dlmb_bram_if_cntlr_memory 0x00000000 0x20000>,
         <0x00000000 &microblaze_riscv_0_local_memory_dlmb_bram_if_cntlr 0x00000000 0x20000>,
         <0x00020000 &myip_pong2_0 0x00020000 0x1000>,
         <0x40000000 &axi_gpio_0 0x40000000 0x10000>,
         <0x40600000 &axi_uartlite_0 0x40600000 0x10000>,
         <0x41c00000 &axi_timer_0 0x41c00000 0x10000>,
         <0x44a00000 &apb_led_ctl_0 0x44a00000 0x10000>;
  #ranges-address-cells = <0x1>;
  #ranges-size-cells = <0x1>;
 };
};
