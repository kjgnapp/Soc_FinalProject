...
void start_pwm_beep() {
    xil_printf("Initializing PWM output (softer tone)...\r\n");

    const u32 PWM_FREQ_HZ = 100;        // Frequency (lower = less piercing)
    const u32 DUTY_PERCENT = 10;        // Lower duty = quieter
    const u32 AXI_CLK_HZ = XPAR_AXI_TIMER_0_CLOCK_FREQUENCY;

    u32 period_cycles = (AXI_CLK_HZ / PWM_FREQ_HZ) - 2;
    u32 high_time_cycles = (period_cycles * DUTY_PERCENT) / 100;
    ...
    Xil_Out32(TIMER_BASEADDR + TLR1_OFFSET, high_time_cycles);

    // Set LOAD0 and LOAD1 to move TLR values into counters
    u32 tcsr_config = (1 << 5) | (1 << 4) | (1 << 9) | (1 << 2) | (1 << 1); // LOAD, ARHT, PWMA0, GENT, UDT
    Xil_Out32(TIMER_BASEADDR + TCSR0_OFFSET, tcsr_config);
    Xil_Out32(TIMER_BASEADDR + TCSR1_OFFSET, tcsr_config);

    // Clear LOAD bits (required before ENT)
    tcsr_config &= ~(1 << 5);  // clear LOAD bit
    Xil_Out32(TIMER_BASEADDR + TCSR0_OFFSET, tcsr_config);
    Xil_Out32(TIMER_BASEADDR + TCSR1_OFFSET, tcsr_config);

    // Enable timers: ENT, ARHT, PWMA0, GENT, UDT
    tcsr_config |= (1 << 7);  // ENT
    Xil_Out32(TIMER_BASEADDR + TCSR0_OFFSET, tcsr_config);
    Xil_Out32(TIMER_BASEADDR + TCSR1_OFFSET, tcsr_config);
